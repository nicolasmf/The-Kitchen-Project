...
def mainmenu(width, height):
    ...
    # Variables
        # Global
    global can, search_bar, button_search, search_icon, window, favorite_star
    # Core
    ...
    can.create_text(width*0.93, height*0.16, text = "Favorites", fill = "black", font = ("Calibri light",15))
        ...
        # Search bar
    search_bar.pack()
    search_bar.place(x = width*0.25, y = height*0.13 , height = 40, width = height*0.7)
    search_bar.insert(0, "Type something here...")
    search_bar.bind("<FocusIn>", func = lambda e: search_bar.delete('0', "end")) 
    # \|/ Focus pas obligatoire à l'arrivé du mainmenu
    #search_bar.focus() 
    
        # Image
    button_search.pack()
    button_search.place(x = width*0.71, y = height*0.14)
